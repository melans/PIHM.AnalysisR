...
pbsPIHM.sensitivity  <-
    ...
    trials <- readtrial(fn=paste(projectname,".",'sens',sep=''));
   ...
   message('\n\nStart calibration processes\t',Sys.time(),'\n');
    ...
    for (ip in 1:npara){    #loop for each parameter in .trial file.
        ...
        for (ivalue in 1:nvalue){
            ...
            prjfile = paste0(fn,'.prj');
            tmpprj['CALIB'] = calibpath;
            writeprj(tmpprj, fn = prjfile)
                Sys.sleep(0.01)
            ...
            pbscmd=paste('./pihm', '-p ',prjfile, '-o ', fn, projectname,  sep=' ')
