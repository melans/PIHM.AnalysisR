...
#    outdir <- file.path(projlist,outdirname);
    ...
    for (i in 1:np ) {
        message(i,'/',np,'\n');
        calib=as.matrix(readcalib(bak=TRUE,folder=projlist[i])$value) #read calib from output folder
        if (i>1){
        ...
        if (!reload){
            ...
            Q<- qq[,id];
            qtime=as.POSIXlt(strptime(time(Q),"%Y-%m-%d"))      # convert time to day format
            time(Q)=qtime;
            Qlist[[i]] <- Q;
        }
