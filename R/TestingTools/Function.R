...
Dinf=0.1
    ...
    yname=paste(key,ff)
 library(ggplot2)
        imagecontrol(path='./Run', fn=paste(key,'.jpg'), wd=30,ht=24,res=100 ,
                     bg='grey')
        par(mfrow=c(2,2))
...
# 
#    col=heat.colors(nf)
#    imagecontrol(path='./', fn=paste(key,'_effK.jpg'))
#    plot.zoo(x = mke, ylab = 'Effective K',xlab='Se', main = paste('Effective Conductivity vs.',key), col = col, screens = 1)
#    legend(x = "topright", legend = yname, lty = 1,col =col )
#     grid()
#    dev.off();
#    
#    imagecontrol(path='./', fn=paste(key,'_Gradient.jpg'))
#    plot.zoo(x = mG, ylab = 'Gradient',xlab='Se', main = paste('Gradient vs.',key), col = col, screens = 1)
#    legend(x = "topright", legend = yname, lty = 1,col =col )
#     grid()
#    dev.off();
#    imagecontrol(path='./', fn=paste(key,'_q.jpg'))
#    plot.zoo(x = mQ, ylab = 'Flow rate',xlab='Se', main = paste('Flow rate vs.',key), col = col, screens = 1)
#    legend(x = "topright", legend = yname, lty = 1,col =col )
#     grid()
#    dev.off();
}
